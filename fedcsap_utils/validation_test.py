...
def validation_test_fun(helper, network, given_test_loader=None, is_poisonous=False, adv_index=-1, tqdm_disable=True, num_classes=10):
    ...
    with torch.no_grad():
        for batch_id, batch in enumerate(test_loader):
            ...
            for cl in range(num_classes):
                ...
                tp_by_class[cl] += ((pred_flat == cl) & (targets_flat == cl)).sum().item()
                fp_by_class[cl] += ((pred_flat == cl) & (targets_flat != cl)).sum().item()
                fn_by_class[cl] += ((pred_flat != cl) & (targets_flat == cl)).sum().item()
    ...
    for class_label in range(num_classes):
        ...
        precision_den = tp_by_class[class_label] + fp_by_class[class_label]
        recall_den = tp_by_class[class_label] + fn_by_class[class_label]
        precision = tp_by_class[class_label] / precision_den if precision_den > 0 else 0.
        recall = tp_by_class[class_label] / recall_den if recall_den > 0 else 0.
        if precision + recall > 0:
            f1_by_class[class_label] = 100. * (2 * precision * recall) / (precision + recall)
        else:
    ...
    if validation_metric == 'LIPC':
        ...
        return f1_by_class, f1_by_class, count_per_class
