...
# 实验四：常规恶意攻击防御能力（PATHMNIST）
# 运行方式改为：从配置表读取 run_xxx 配置，并按 run 区间启动。
# 仍然保留“每组三个并行，组间 wait”的并行策略。
#
# 用法：
#   bash scripts/exp_04_pathmnist.sh 81 152
#   bash scripts/exp_04_pathmnist.sh run_081 run_152
#
# 可选环境变量：
#   MAX_PARALLEL=3
#   CONFIG_FILE=scripts/configs/exp_04_pathmnist_runs.csv
#   PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
#   DRY_RUN=1   # 只打印命令，不实际启动

MAX_PARALLEL=${MAX_PARALLEL:-3}
CONFIG_FILE=${CONFIG_FILE:-scripts/configs/exp_04_pathmnist_runs.csv}
...
normalize_run_id() {
  ...
  if [[ "${input}" =~ ^run_([0-9]+)$ ]]; then
    ...
    echo "Invalid run id: ${input}. Use 81 or run_081 format." >&2
    exit 1
  fi
}

START_INPUT=${1:-run_081}
END_INPUT=${2:-run_152}
